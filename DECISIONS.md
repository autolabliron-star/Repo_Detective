...
because the anonymous limit of 60 requests per hour is the real constraint without a token.
...
* **Pause and hard stop:** the agent can ask for more budget at any time, and at the latest when
  ...
  instruction the model may ignore. The paused state is saved, so the same decision can be
  answered from the CLI or the web UI.
...
* **Tools take explicit owner/repo values,** so "check the biggest fork" is the same tools pointed
  ...
  directory, so a `Readme.md` is not mistaken for a missing README because of case sensitivity.
...
* **Lessons from testing:** a 1024-token output cap cut off the verdict JSON and the retry loop
  ...
  GitHub counts `304 Not Modified` responses against the anonymous limit, so a one-hour cache
  protects the quota and ETags only take over after it. Providers differ in what they accept for
  function calling; the client reads the first HTTP 400 and adjusts temperature, the token-limit
  parameter name, forced tool choice, and reasoning effort.
...
remaining budget; new verdicts are saved as v2, v3, and so on.
